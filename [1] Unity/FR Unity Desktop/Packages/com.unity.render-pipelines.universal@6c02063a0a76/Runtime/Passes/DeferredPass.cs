        ...
        internal void Render(RenderGraph renderGraph, ContextContainer frameData, TextureHandle color, TextureHandle depth, TextureHandle[] gbuffer)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData, profilingSampler))
            {
                ...
                if (ShadingRateInfo.supportsPerImageTile &&
                    frameData.Contains<VrsContextData>())
                {
                    var vrs = frameData.Get<VrsContextData>();
                    if (vrs.shadingRateImage.IsValid())
                    {
                        builder.SetShadingRateImageAttachment(vrs.shadingRateImage);
